...
const LeftSidebar = ({ isOpen, onClose, logoSrc }) => {
  ...
  // Helper to safely get min/max price strings
  ...
  return (
    <>
      ...
      <div
        ref={sidebarRef}
        className={`fixed top-0 left-0 h-full bg-white shadow-2xl z-50 w-80 transform transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : '-translate-x-full'} flex flex-col`}
      >
        {/* Logo Section */}
          ...
          <div className="px-4 py-6">
            ...
            <div className="space-y-2">
              {Object.entries(categories).map(([categoryKey, category]) => {
                const { min, max } = getPriceRange(category.products);
                return (
                  <div 
                    key={categoryKey}
                          ...
                          <div>
                            ...
                            <div className="space-y-2 max-h-64 overflow-y-auto">
                              {category.products.slice(0, 8).map((product, index) => {
                                const price = (typeof product.originalPrice === 'number' && !isNaN(product.originalPrice)) 
